...
class claseDAO {
  ...
  function eliminarClase($codigo) {
    ...
    try {
      $conn = $conexion->Conectar();
      $stmt = $conn->prepare("DELETE FROM productos WHERE codigo= $codigo");
  ...
  function agregarClases($nombre, $precio, $categoria) {
    $conexion = new Conexion('localhost', 'root', '', 'tiendax');
    try {
        $conn = $conexion->Conectar(); 
        $agregar = $conn->prepare("INSERT INTO productos (`nombre`, `precio`, `categoria`) VALUES (?, ?, ?)");
